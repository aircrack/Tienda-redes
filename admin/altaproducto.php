			...
			if ((($_FILES["file"]["type"] == "image/gif")
		  	...
		  	if ($_FILES["file"]["error"] > 0){
		    	...
		    	if(file_exists("../productos/".$random.'_'.$_FILES["file"]["name"])){
					...
					$precio=$_POST['precio'];
					$Sql="insert into productos (descripcion,categoria,precio,imagen) values(
							'".$producto."',
							'".$descripcion."',
							".$precio.",
							'".$imagen."')";
					mysql_query($Sql);	
echo "<script language='javascript'> alert('Se a enviado a la BD el nuevo producto');  document.location=('index.php');</script>"; 					 
